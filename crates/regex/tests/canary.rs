...
use common::{LINUX_SYSLOG, canary_only};
use fusion_regex::canary::{CanaryConfig, run};
use fusion_regex::{
    CompileError, Engine, EngineChoice, Limits, MatchError, Options, RedosPolicy, Regex,
};

/// Limits with a match limit far below the canary's work budget, so the match limit trips.
fn tight() -> Limits {
    Limits {
        match_limit: 10_000,
        ..Limits::default()
...
/// [`canary_only`] under [`tight`] limits.
fn tight_canary() -> Options {
    Options {
        limits: tight(),
        ..canary_only(CanaryConfig::default())
    }
}
...
fn canary_rejects_nested_quantifier_by_match_limit_when_it_is_tight() {
    ...
    let trip = run(r"(a+)+$", &CanaryConfig::default(), &tight())
        .unwrap()
        .expect("should trip");
...
fn canary_accepts_bounded_digits() {
    assert_eq!(
        run(r"^\d{1,3}$", &CanaryConfig::default(), &tight()).unwrap(),
        None
    );
}
...
fn canary_covers_backtracking_only_syntax_and_a_literal_prefix() {
    ...
    let trip = run(r"x(?=a)(a+)+$", &CanaryConfig::default(), &tight()).unwrap();
    assert!(trip.is_some());
}
...
fn canary_uses_the_literal_sequence_as_a_prefix() {
    ...
    let trip = run(r"ERROR: (a+)+$", &CanaryConfig::default(), &tight()).unwrap();
    assert!(trip.is_some());
}
...
fn canary_reports_syntax_errors_as_compile_errors() {
    let err = run(r"a(", &CanaryConfig::default(), &tight()).unwrap_err();
...
fn canary_is_skipped_for_patterns_on_the_linear_engine() {
    ...
    let re = Regex::with_options(r"(a+)+$", &tight_canary()).unwrap();
...
fn canary_clamps_sizes_to_the_input_limit_rather_than_skipping() {
    ...
    let trip = run(r"(a+)+$", &CanaryConfig::default(), &limits)
        .unwrap()
        .expect("should trip");
...
fn default_options_run_every_layer_and_reject() {
    ...
    let err =
        Regex::with_options(r"(?=a)(a+)+$", &canary_only(CanaryConfig::default())).unwrap_err();
    assert!(matches!(err, CompileError::CanaryTripped(_)), "{err:?}");
