...
use pcre2_sys::{
    ...
    PCRE2_ERROR_MATCHLIMIT, PCRE2_ERROR_NOMATCH, PCRE2_INFO_CAPTURECOUNT,
    PCRE2_INFO_NAMECOUNT, PCRE2_INFO_NAMEENTRYSIZE, PCRE2_INFO_NAMETABLE,
    PCRE2_NEVER_BACKSLASH_C, PCRE2_NO_UTF_CHECK, PCRE2_UCP, PCRE2_UTF,
    pcre2_code_8, pcre2_code_free_8, pcre2_compile_8, pcre2_compile_context_create_8,
    ...
    pcre2_match_data_free_8, pcre2_pattern_info_8, pcre2_set_depth_limit_8,
    pcre2_set_heap_limit_8, pcre2_set_match_limit_8, pcre2_set_max_pattern_length_8,
...
impl Pcre2Regex {
    ...
    pub(crate) fn compile(pattern: &str, limits: &Limits) -> Result<Self, CompileError> {
        ...
        let options =
            PCRE2_UTF | PCRE2_UCP | PCRE2_NEVER_BACKSLASH_C | PCRE2_DOLLAR_ENDONLY;
    ...
    fn finish(code: NonNull<pcre2_code_8>, limits: &Limits) -> Result<Self, CompileError> {
        ...
        Ok(Self { code, match_context, capture_count, names })
    }
...
fn pattern_info_u32(code: NonNull<pcre2_code_8>, what: u32) -> Result<u32, CompileError> {
    ...
    let rc = unsafe {
        pcre2_pattern_info_8(code.as_ptr(), what, (&mut out as *mut u32).cast())
    };
    if rc != 0 {
        return Err(CompileError::Internal { code: rc, message: error_message(rc) });
    }
    Ok(out)
}
...
fn match_error(rc: CInt) -> MatchError {
    match rc {
        ...
        other => MatchError::Engine { code: other, message: error_message(other) },
    }
}
