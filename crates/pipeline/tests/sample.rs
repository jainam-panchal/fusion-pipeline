...
fn random_gives_a_redelivered_record_the_same_verdict() {
    ...
    assert!(
        !first_pass.is_empty() && first_pass.len() < ids.len(),
        "{first_pass:?}"
    );
    ...
    assert_eq!(
        h.ids("out"),
        twice,
        "every kept id kept again, every dropped id dropped again"
    );
    h.finish();
}
...
fn every_nth_counts_a_redelivered_record_again_because_sampling_is_per_delivery() {
    ...
    assert_eq!(
        h.state
            .get("ingest:acme:keep_some:sample:count")
    ...
    assert_eq!(
        h.state.keys(),
        vec!["ingest:acme:keep_some:sample:count"],
        "no key per record"
    );
    h.finish();
}
...
fn consistent_at_fifty_percent_keeps_or_drops_every_record_of_a_host_together() {
    ...
    assert_eq!(
        kept.len(),
        kept_hosts.len() * 3,
        "every kept host has all 3 copies"
    );
    assert!(
...
/// Records missing the key share one `null` value whose flip point is fixed: about 32 for
/// a one-field key. So at 50 they are all kept and at 30 all dropped.
#[test]
fn consistent_decides_all_records_missing_the_key_field_together() {
    for (percent, kept) in [("50", 20), ("30", 0)] {
        let yaml = CONSISTENT_HALF.replace("percent: 50", &format!("percent: {percent}"));
        let h = start(&yaml, 1);
        for id in 1..=20 {
            assert_eq!(
        ...
        assert_eq!(h.ids("out").len(), kept, "percent {percent}");
        h.finish();
    }
}
...
"#;
    ...
    let kept = h.ids("out").len();
    assert!(
        (700..=1_300).contains(&kept),
        "kept {kept}, expected about 1,000"
    );
    h.finish();
}
