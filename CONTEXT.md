...
**Expectation**:
What the loghub producer expects of one message it published: the subjects its line must reach (the main one only when the POC config's `sample` node keeps the line in that cycle), `dedupe` as the drop planned for a deliberate duplicate, the attributes the structured CSV says extraction lifts, and what the POC config's `edit` and `lua` nodes write. Written only after the message's `PubAck`, one per line of the expectations file, and never computed by running the pipeline.
