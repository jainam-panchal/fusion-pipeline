...
- **Windows are measured in ingestion time** (`observed_time_unix_nano`, then `time_unix_nano`, else the worker clock), not on arrival at the stage, so a redelivered record gets the same answer whenever it comes back. The NATS source stamps the JetStream publish time on records with no timestamp, so the clock fallback is reached only from a source that does not. The three stated exceptions to "same answer on redelivery" are `sample`'s `every_nth`, which counts deliveries (spec amendment 2026-09-15, issue #7), a `lua` script's upvalues, which are per worker VM and reset when a `memory` error rebuilds it, and a `lua` script writing a clock reading into a time field, which is ingestion time for every stateful node downstream of it (spec amendments 2026-09-16, issue #8).
