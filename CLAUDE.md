...
- **Windows are measured in ingestion time** (`observed_time_unix_nano`, then `time_unix_nano`), never on arrival at the stage, so a redelivered record gets the same answer whenever it comes back.
