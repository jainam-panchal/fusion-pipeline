...
- **Decisions read `Meta`, never the payload, and `Meta` never enters the payload** (ADR 0005). `Meta::resolve` builds `Meta` (record id, tenant, ingestion time, delivery count) once at intake, and the engine hands it to stages read-only on the context. The tenant and the ingestion time come from the source's `Arrival` only: no arrival tenant is `unknown`, no arrival time is the worker clock, and no key such as `resource.tenant.id` or a record time field is ever read for them, not even as a fallback. The only payload fields the pipeline reads for itself are `id` and `kind`, at intake. Metric labels, state keys, windows and every stored record id read `Meta`. The pipeline never writes a value of its own into a record: no source stamps, no sink projects. `Meta` leaves on the wire as the `Fusion-Tenant`, `Fusion-Ingestion-Time` and `Fusion-Ingestion-Time-Kind` headers, and a config reads it through the read-only `meta.*` paths; `edit copy {from: meta.<field>, ...}` is the only way one enters a record. Every record field is payload, `id`, `kind`, the tenant and the time fields included: stages may write or remove any of them within core's write rules, and the sink writes the record as the last stage left it.
...
- **Only `kind: log` is processed, decided at intake.** A record arriving as `metric` or `span` is counted and rejected before any stage runs. Past intake `kind` is payload like `id`: a stage may set another kind, and the sink writes it (ADR 0005).
