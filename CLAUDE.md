...
- **Closed sets are declared through `closed_set!`** (`DropReason`, `Metric`, `EngineLabel`, `LuaErrorKind`, `EditOp`, `EditCause`, `Kind`, the path roots), so a value cannot be left out of `ALL`. `DropReason` is the `reason` label on `records_dropped_total`; adding a variant to any spec set is a spec amendment. A metric is declared with its instrument, and the recorder's counter and histogram calls take only their own kind.
